...
import type { FacturaVentaDocument, VentaMaterialItem, FacturaVentaFormData } from "@/schemas/venta";
import type { CompanyProfileDocument } from "@/schemas/company";
import type { MaterialDocument } from "@/schemas/material";
import { FacturaVentaFormSchema } from "@/schemas/venta";
...
export default function EditFacturaVentaPage() {
    ...
    const fetchInvoiceData = async () => {
      ...
      try {
        ...
        if (invoiceSnap.exists()) {
          const data = invoiceSnap.data() as FacturaVentaDocument;
          setInvoice(data);
  ...
  const handleItemFieldChange = (index: number, field: keyof VentaMaterialItem, value: string | number) => {
  ...
  const handleAddNewItemToInvoice = (data: VentaMaterialItemFormData) => {
    ...
    // Stock check
    const stockDisponible = selectedMaterial.stock || 0;
    const pesoEnCarrito = editableItems.filter(item => item.materialId === newItem.materialId).reduce((sum, item) => sum + item.peso, 0);
    if ((pesoEnCarrito + newItem.peso) > stockDisponible) {
        toast({ variant: "destructive", title: "Stock Insuficiente", description: `No puede vender más de ${stockDisponible} kg de ${selectedMaterial.name}.` });
        return;
    }
  ...
  const handleUpdateInvoice = async (formData: FacturaVentaFormData) => {
    ...
    try {
      ...
      for (const [materialId, change] of stockAdjustments.entries()) {
        if (change !== 0) {
      ...
      const updatedData: Partial<FacturaVentaDocument> = {
      ...
      router.push("/dashboard/gestion-material/facturas-venta");
  ...
  const triggerSaveInvoice = async () => {
    ...
    if (isValid) {
        ...
        handleUpdateInvoice(form.getValues() as FacturaVentaFormData);
    }
  };
  ...
  const printFacturaPreview = () => {
    ...
    if (previewElement && invoice && companyProfile) {
      ...
      printWindow.document.write('<html><head><title>Factura Venta N° '+ invoice.numeroFactura +'</title>');
      ...
      printHtml += `<p><span>Factura de Venta N°:</span> <span style="font-weight: bold;">${invoice.numeroFactura}</span></p>`;
      printHtml += `<p><span>Fecha y Hora:</span> <span>${formatDateWithTimeForDisplay(form.watch("fecha"))}</span></p>`;
      printHtml += '</div>';

      const watchedUsuarioNombre = form.watch("clienteNombre");
    ...
    <div className="container py-8 px-4 md:px-6">
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-headline text-primary flex items-center">
            ...
            Editar Factura de Venta N° {invoice.numeroFactura}
          </CardTitle>
          <CardDescription>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <div className="relative">
                          ...
                          <FormControl>
                            <Input placeholder="Ej: Empresa XYZ" {...field} value={field.value ?? ""} className="pl-10" />
                          </FormControl>
                        </div>
                        <FormMessage />
                      </FormItem>
                    ...
                    <div className="flex justify-between items-center mb-3">
                     ...
                     <div className="p-4 border rounded-md bg-background text-sm max-h-[50vh] overflow-y-auto">
                        ...
                        <div className="invoice-info my-2 text-xs">
                            <p><strong>Factura Venta N°:</strong> <span className="font-semibold float-right">{invoice.numeroFactura}</span></p>
                        ...
                        {form.watch("clienteNombre") && (
                            <>
                            <div className="section-title text-center font-bold my-2 text-xs">Cliente</div>
                         ...
                         <div className="section-title text-center font-bold my-2 text-xs">Detalle de la Venta</div>
      ...
      {isItemFormOpen && availableMaterials.length > 0 && (
        <VentaMaterialItemForm
            ...
            currentSaleItems={editableItems}
        />
      )}
